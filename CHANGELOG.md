...
### Added
- `--auto` flag for `/cgsd:new-project` — runs research → requirements → roadmap automatically after config questions. Expects idea document via @ reference (e.g., `/cgsd:new-project --auto @prd.md`)
...
- Research decision from `/cgsd:new-milestone` now persists to config.json
...
- `/cgsd:reapply-patches` command to merge local modifications back after GSD updates
...
- `/cgsd:update` workflow now notifies about backed-up local patches and suggests `/cgsd:reapply-patches`
...
### Fixed
- Execute-phase now correctly spawns `cgsd-executor` subagents instead of generic task agents
...
### Fixed
- **CRITICAL:** Prevent API keys from being committed via `/cgsd:map-codebase` (#429)
...
- Auto-create `config.json` when missing during `/cgsd:settings` (#264)
- `/cgsd:update` respects local vs global install location
...
### Fixed
- CONTEXT.md from `/cgsd:discuss-phase` now properly flows to all downstream agents (researcher, planner, checker, revision loop)
...
### Removed
- `/cgsd:whats-new` command — use `/cgsd:update` instead (shows changelog with cancel option)
...
### Added
- `/cgsd:join-discord` command to quickly access the GSD Discord community invite link
...
- OpenCode commands use flat structure (`command/cgsd-help.md`) matching OpenCode's expected format
...
- Removed stray backticks in `/cgsd:new-project` output
...
- **Codebase Intelligence System** — Removed due to overengineering concerns
  - Deleted `/cgsd:analyze-codebase` command
  - Deleted `/cgsd:query-intel` command
...
### Added
- **Model Profiles** — `/cgsd:set-profile` for quality/balanced/budget agent configurations
- **Workflow Settings** — `/cgsd:settings` command for toggling workflow behaviors interactively
...
- All hooks now use `gsd-` prefix for consistency (statusline.js → cgsd-statusline.js)
...
- `/cgsd:new-project` now asks about git tracking during initial setup, letting you opt out of committing planning docs from the start
...
### Added
- **Quick Mode** (`/cgsd:quick`) — Execute small, ad-hoc tasks with GSD guarantees but skip optional agents (researcher, checker, verifier). Quick tasks live in `.planning/quick/` with their own tracking in STATE.md.
...
### Added
- `--gaps-only` flag for `/cgsd:execute-phase` — executes only gap closure plans after verify-work finds issues, eliminating redundant state discovery
...
- `/cgsd:update` shows changelog and asks for confirmation before updating, with clear warning about what gets replaced

## [1.6.0] - 2026-01-17

### Changed
- **BREAKING:** Unified `/cgsd:new-milestone` flow — now mirrors `/cgsd:new-project` with questioning → research → requirements → roadmap in a single command
- Roadmapper agent now references templates instead of inline structures for easier maintenance

### Removed
- **BREAKING:** `/cgsd:discuss-milestone` — consolidated into `/cgsd:new-milestone`
- **BREAKING:** `/cgsd:create-roadmap` — integrated into project/milestone flows
- **BREAKING:** `/cgsd:define-requirements` — integrated into project/milestone flows
- **BREAKING:** `/cgsd:research-project` — integrated into project/milestone flows

### Added
- `/cgsd:verify-work` now includes next-step routing after verification completes
...
- Next-step suggestions now consistently recommend `/cgsd:discuss-phase` before `/cgsd:plan-phase` across all routing paths
...
- **BREAKING:** Removed `/cgsd:execute-plan` command (use `/cgsd:execute-phase` instead)
...
- Removed `/cgsd:research-phase` from next-step suggestions (use `/cgsd:plan-phase` instead)

## [1.5.22] - 2025-01-16

### Added
- Statusline update indicator — shows `⬆ /cgsd:update` when a new version is available
...
### Changed
- **Unified `/cgsd:new-project` flow** — Single command now handles questions → research → requirements → roadmap (~10 min)
- Simplified README to reflect streamlined workflow: new-project → plan-phase → execute-phase
- Added optional `/cgsd:discuss-phase` documentation for UI/UX/behavior decisions before planning
...
### Changed
- `/cgsd:discuss-phase` redesigned with intelligent gray area analysis — analyzes phase to identify discussable areas (UI, UX, Behavior, etc.), presents multi-select for user control, deep-dives each area with focused questioning
...
- `/cgsd:plan-phase` now integrates research — spawns `cgsd-phase-researcher` before planning unless research exists or `--skip-research` flag used

## [1.5.18] - 2026-01-16

### Added
- **Plan verification loop** — Plans are now verified before execution with a planner → checker → revise cycle
  - New `cgsd-plan-checker` agent (744 lines) validates plans will achieve phase goals
...
- **Dedicated planner agent** — `cgsd-planner` (1,319 lines) consolidates all planning expertise
...
### Changed
- `/cgsd:plan-phase` refactored to thin orchestrator pattern (310 lines)
  - Spawns `cgsd-planner` for planning, `cgsd-plan-checker` for verification
  - User sees status between agent spawns (not a black box)
- Planning references deprecated with redirects to `cgsd-planner` agent sections
...
### Added
- New `/cgsd:update` command — check for updates, install, and display changelog of what changed (better UX than raw `npx get-shit-done-cc`)
...
- New `cgsd-debugger` agent (990 lines) with scientific debugging methodology, hypothesis testing, and 7+ investigation techniques
- New `cgsd-codebase-mapper` agent for brownfield codebase analysis
- Research subagent prompt template for context-only spawning

### Changed
- `/cgsd:research-phase` refactored to thin orchestrator — now injects rich context (key insight framing, downstream consumer info, quality gates) to cgsd-researcher agent
- `/cgsd:research-project` refactored to spawn 4 parallel cgsd-researcher agents with milestone-aware context (greenfield vs v1.1+) and roadmap implications guidance
- `/cgsd:debug` refactored to thin orchestrator (149 lines) — spawns cgsd-debugger agent with full debugging expertise
- `/cgsd:new-milestone` now explicitly references MILESTONE-CONTEXT.md

### Deprecated
- `workflows/research-phase.md` — consolidated into cgsd-researcher agent
- `workflows/research-project.md` — consolidated into cgsd-researcher agent
- `workflows/debug.md` — consolidated into cgsd-debugger agent
- `references/research-pitfalls.md` — consolidated into cgsd-researcher agent
- `references/debugging.md` — consolidated into cgsd-debugger agent
- `references/debug-investigation.md` — consolidated into cgsd-debugger agent

## [1.5.15] - 2025-01-15

### Fixed
- **Agents now install correctly** — The `agents/` folder (cgsd-executor, cgsd-verifier, cgsd-integration-checker, gsd-milestone-auditor) was missing from npm package, now included

### Changed
- Consolidated `/cgsd:plan-fix` into `/cgsd:plan-phase --gaps` for simpler workflow
- UAT file writes now batched instead of per-response for better performance

## [1.5.14] - 2025-01-15

### Fixed
- Plan-phase now always routes to `/cgsd:execute-phase` after planning, even for single-plan phases

## [1.5.13] - 2026-01-15

### Fixed
- `/cgsd:new-milestone` now presents research and requirements paths as equal options, matching `/cgsd:new-project` format
...
- `progress` now correctly routes to `/cgsd:discuss-milestone` when between milestones (Route F)
...
### Added
- Milestone audit system (`/cgsd:audit-milestone`) for verifying milestone completion with parallel verification agents
...
- Execute-phase now recommends `/cgsd:audit-milestone` when milestone completes
...
### Changed
- `cgsd-executor` subagent color changed from red to blue

## [1.5.7] - 2025-01-15

### Added
- `cgsd-executor` subagent: Dedicated agent for plan execution with full workflow logic built-in
- `cgsd-verifier` subagent: Goal-backward verification that checks if phase goals are actually achieved (not just tasks completed)
...
### Changed
- execute-plan and execute-phase now spawn `cgsd-executor` subagent instead of using inline workflow
...
### Fixed
- Updated remaining `general-purpose` subagent references to use `cgsd-executor`
...
### Removed
- Dead `/cgsd:status` command (referenced abandoned background agent model)
...
### Added
- New `/cgsd:research-project` command for pre-roadmap ecosystem research — spawns parallel agents to investigate stack, features, architecture, and pitfalls before you commit to a roadmap
- New `/cgsd:define-requirements` command for scoping v1 requirements from research findings — transforms "what exists in this domain" into "what we're building"
...
### Added
- New `/cgsd:whats-new` command shows changes since your installed version
...
### Added
- `/cgsd:debug` command for systematic debugging with persistent state
...
### Added
- Parallel phase execution via `/cgsd:execute-phase`
- Parallel-aware planning in `/cgsd:plan-phase`
- `/cgsd:status` command for parallel agent monitoring
...
### Added
- `/cgsd:add-todo` and `/cgsd:check-todos` for mid-session idea capture
...
### Added
- `/cgsd:resume-task` for resuming interrupted subagent executions
...
### Added
- `/cgsd:verify-work` for conversational UAT validation
- `/cgsd:plan-fix` for fixing UAT issues
- UAT issues template
...
### Added
- `/cgsd:map-codebase` documentation in help and README

## [1.3.0] - 2025-12-17

### Added
- `/cgsd:map-codebase` command for brownfield project analysis
...
- `/cgsd:research-phase` for niche domain ecosystem discovery
- `/cgsd:research-project` command with workflow and templates
- `/cgsd:create-roadmap` command with research-aware workflow
...
- Core slash commands: `/cgsd:new-project`, `/cgsd:discuss-phase`, `/cgsd:plan-phase`, `/cgsd:execute-phase`
