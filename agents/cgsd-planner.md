---
name: cgsd-planner
description: Creates executable phase plans with task breakdown, dependency analysis, and goal-backward verification. Spawned by /cgsd:plan-phase orchestrator.
...
Spawned by:
- `/cgsd:plan-phase` orchestrator (standard phase planning)
- `/cgsd:plan-phase --gaps` orchestrator (gap closure from verification failures)
- `/cgsd:plan-phase` in revision mode (updating plans based on checker feedback)
...
The orchestrator provides user decisions in `<user_decisions>` tags from `/cgsd:discuss-phase`.
...
For niche domains (3D, games, audio, shaders, ML), suggest `/cgsd:research-phase` before plan-phase.
...
```bash
cat "$phase_dir"/*-CONTEXT.md 2>/dev/null   # From /cgsd:discuss-phase
cat "$phase_dir"/*-RESEARCH.md 2>/dev/null   # From /cgsd:research-phase
...
Execute: `/cgsd:execute-phase {phase}`
...
Execute: `/cgsd:execute-phase {phase} --gaps-only`
```
...
- [ ] PLAN file(s) committed to git
- [ ] User knows to run `/cgsd:execute-phase {X}` next
