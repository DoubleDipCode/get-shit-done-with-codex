...
Todos are captured during work sessions with /cgsd:add-todo.

---

Would you like to:

1. Continue with current phase (/cgsd:progress)
2. Add a todo now (/cgsd:add-todo)
```
...
- `/cgsd:check-todos` → show all
- `/cgsd:check-todos api` → filter to area:api only
</step>
...
- `/cgsd:check-todos [area]` to filter by area
- `q` to exit
```
...
- options:
  - "Work on it now" — move to done, start working
  - "Create a phase" — /cgsd:add-phase with this scope
  - "Brainstorm approach" — think through before deciding
  - "Put it back" — return to list
</step>
...
**Create a phase:**
Display: `/cgsd:add-phase [description from todo]`
