...
- `commands/cgsd/` will be wiped and replaced
- `get-shit-done/` will be wiped and replaced
- `agents/cgsd-*` files will be replaced

(Paths are relative to your install location: `~/.claude/` for global, `./.claude/` for local)

Your custom files in other locations are preserved:
- Custom commands not in `commands/cgsd/` ✓
- Custom agents not prefixed with `gsd-` ✓
- Custom hooks ✓
- Your CLAUDE.md files ✓

If you've modified any GSD files directly, they'll be automatically backed up to `gsd-local-patches/` and can be reapplied with `/cgsd:reapply-patches` after the update.
```
...
Run /cgsd:reapply-patches to merge your modifications into the new version.
```
